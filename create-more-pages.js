...
// 2. Product Details Page: /farmer/marketplace/products/[slug]/page.tsx
...
export default function ProductDetailPage() {
  ...
  const fetchProductBySlug = async (productSlug: string) => {
    setLoading(true);
    try {
      const res = await fetch(\`http://localhost:4000/api/v1/marketplace/products/\${productSlug}\`);
...
writeWithDir(path.join(__dirname, 'apps', 'web', 'src', 'app', 'farmer', 'marketplace', 'products', '[slug]', 'page.tsx'), productDetailsPage);
console.log('Created product details page.');
