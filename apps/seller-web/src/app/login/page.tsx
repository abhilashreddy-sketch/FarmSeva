...
export default function SellerLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        setStatus(`Seller logged in successfully as ${data.data.user.fullName}`);
      } else {
  ...
  return (
    <div className="max-w-md mx-auto py-10 space-y-6">
      <div className="text-center space-y-2">
        <h1 className="text-2xl font-black text-slate-900">Seller Sign In</h1>
        <p className="text-xs text-slate-600 font-medium">Access your Agri Dealer Shop Console</p>
      </div>
      ...
      <form onSubmit={handleLogin} className="p-6 bg-white border border-slate-200 rounded-2xl space-y-4 shadow-sm">
        ...
        <button type="submit" className="w-full py-2.5 bg-amber-600 text-white font-bold text-xs rounded-xl hover:bg-amber-700">
          Sign In to Seller App
        </button>
      </form>

      <p className="text-xs text-center text-slate-600">
        New dealer? <Link href="/register" className="text-amber-700 font-bold hover:underline">Register Shop</Link>
      </p>
    </div>
  );
}
