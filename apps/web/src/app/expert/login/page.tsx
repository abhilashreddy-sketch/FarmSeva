...
import { Mail, Lock, Eye, EyeOff, Stethoscope, ArrowRight, ShieldCheck } from 'lucide-react';
...
function ExpertLoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      ...
      setErrorMsg(result.error || 'Expert login failed. Please check your credentials.');
    }
  };
  ...
  return (
    <div className="min-h-[80vh] flex flex-col justify-center py-10 px-4 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md space-y-4 text-center">
        ...
        <h1 className="text-3xl font-black text-slate-900 tracking-tight">FARM SEVA EXPERT</h1>
        <p className="text-sm font-medium text-slate-600">
          Crop Expert & Pathologist Workstation — Review disease diagnostics and issue agronomic prescriptions.
        </p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              <Link href="/forgot-password" className="text-xs font-semibold text-sky-700 hover:underline">
                Forgot Password?
              </Link>
            </div>
...
export default function ExpertLoginPage() {
  return (
    <Suspense fallback={<div className="p-10 text-center font-bold text-slate-600">Loading Expert Desk...</div>}>
      <ExpertLoginForm />
    </Suspense>
  );
}
