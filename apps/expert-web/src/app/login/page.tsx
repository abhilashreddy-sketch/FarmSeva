...
export default function ExpertLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        setStatus(`Expert logged in successfully as ${data.data.user.fullName}`);
      } else {
  ...
  return (
    <div className="max-w-md mx-auto py-10 space-y-6">
      <div className="text-center space-y-2">
        <h1 className="text-2xl font-black text-slate-900">Expert Sign In</h1>
        <p className="text-xs text-slate-600 font-medium">Access your Agronomist Diagnostic Workstation</p>
      </div>
      ...
      <form onSubmit={handleLogin} className="p-6 bg-white border border-slate-200 rounded-2xl space-y-4 shadow-sm">
        ...
        <div>
          <label className="text-xs font-bold text-slate-700 block mb-1">Expert Email</label>
          <input
            type="email"
      ...
      <p className="text-xs text-center text-slate-600">
        Certified agronomist? <Link href="/register" className="text-sky-700 font-bold hover:underline">Register Profile</Link>
      </p>
    </div>
  );
}
