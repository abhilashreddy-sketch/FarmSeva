...
const sellerSvc = `import { PrismaClient } from '@prisma/client';
...
const adminSvc = `import { PrismaClient } from '@prisma/client';
import { ApiError } from '../middleware/error-middleware';
...
export class AdminMarketplaceService {
  ...
  static async reviewProduct(
    ...
    await logAuditEvent({
      ...
      details: { productId, productName: product.name, status, rejectionReason },
    });
...
fs.writeFileSync(path.join(__dirname, 'apps', 'api', 'src', 'services', 'seller-marketplace-service.ts'), sellerSvc, 'utf8');
fs.writeFileSync(path.join(__dirname, 'apps', 'api', 'src', 'services', 'admin-marketplace-service.ts'), adminSvc, 'utf8');
console.log('Created seller and admin marketplace services.');
